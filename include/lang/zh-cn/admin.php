<?php
/**
 * @Author: Lonelyer <hackkey@qq.com>
 * @link:  http://www.7s.com.cn
 * @Date:   2016-05-12 16:09:52
 * @Last Modified time: 2016-05-17 11:45:03
 * @Packages:   nnCMS
 * @Copyright: Copyright (c) 2016 7s.com.cn.Co.Ltd. All rights reserved.
 */
$LANG['remove'] = '删除';
$LANG['del'] = '删除';
$LANG['delete'] = '删除';
$LANG['user_group_manage'] = '用户组管理';
$LANG['secure_setting'] = '安全设置';
$LANG['goods_manage'] = '商品管理';
$LANG['goods_list'] = '商品列表';
$LANG['goods_add'] = '商品添加';
$LANG['user_add'] = '新增用户';
$LANG['user_edit'] = '用户编辑';
$LANG['update_order'] = '更新排序';
$LANG['goods_remove'] = ' 商品删除';
$LANG['menu_set'] = '导航设置';
$LANG['account_does_not_exist'] = '账户不存在';
$LANG['submit_success'] = '提交成功';
$LANG['listorder_update_success'] = '排序成功更新';
$LANG['operation_failure'] = '操作失败';
$LANG['operation_success'] = '操作成功';
$LANG['delete_faliure'] = '删除失败！';
$LANG['birthday'] = '生日';
$LANG['not_selected_item'] = '未选择项目';
$LANG['add_success'] = '添加成功';
$LANG['add_failure'] = '添加失败';
$LANG['advertise_manage'] = '广告管理';
$LANG['role_name'] = '角色名称';
$LANG['role_description'] = '角色描述';
$LANG['user_status'] = '用户状态';
$LANG['content_manage'] = '内容管理';
$LANG['order_manage'] = '订单管理';
$LANG['user_name'] = '用户名';
$LANG['old_password'] = '原始密码';
$LANG['new_password'] = '新密码';
$LANG['update_passwd_success'] = '修改密码成功，请重新登录！';
$LANG['update_passwd_failure'] = '修改密码失败！';