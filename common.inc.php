...
define('TPL_TRUE_PATH', APP_PATH . '/template/' . TPL_DIR);
 //模板真实路径
// 定义SKIN PATH
...
$spConfig['db_spdb_full_tblname'] = false;
 //是否使用表全名
$spConfig['sp_cache'] = APP_PATH . '/runtime/tmp';
 //临时目录
$spConfig['dispatcher_error'] = "import(SP_PATH.'/Misc/404.php');exit();";
 //404错误页面
...
if (isset($config['template']) && !empty($config['template'])) {
    ...
    $spConfig['dispatcher_error'] = "import(\$GLOBALS['G_SP']['controller_path'].DIRECTORY_SEPARATOR.'error_404.php');\$handle=spClass('error_404');\$handle->handle(\$__controller,\$__action);;exit();";
     //404错误页面
    //自动加载模板
